...
//2070 super
#define NR_CORES 0xff

CONST u8 confusion[512] = {
...
CONST u8 target[OLEN] = "Hire me!!!!!!!!";

GLOBAL void Forward1(u8 *c, u8 *d) {
  for(u8 j = 0; j < ILEN; j++) {
    d[j] = confusion[c[j]];
    c[j] = 0;
  }
...
GLOBAL void Forward2(u8 *c, const u8 *d) {
...
GLOBAL bool Forward3(const u8 *c, u8 *d) {
  bool eq = true;
  for(u8 i = 0; i < OLEN; i++) {
    d[i] = confusion[c[i * 2]] ^ confusion[c[i * 2 + 1] + 256];
    eq &= d[i] == target[i];
  }
  return eq;
}
...
#ifndef CUDA
  __host__ void Forward(u8* results, u32 index) {
#else
  __global__ void Forward(u8* results) {
...
#ifdef CUDA
  cudaMallocManaged(&results, NR_CORES * BATCH_SIZE);
  Forward<<<1, NR_CORES>>>(results);
  cudaDeviceSynchronize();
#else
  results = (u8*) malloc(NR_CORES*BATCH_SIZE);
  for(int i=0;i<NR_CORES;i++)
    Forward(results, i);
#endif
  printf("Execution time = %ld\n", time()-start);

  for(u64 i=0;i<NR_CORES*BATCH_SIZE;i++) {
    if(results[i]) {
      printf("%lx\n", i);
    }
  }
